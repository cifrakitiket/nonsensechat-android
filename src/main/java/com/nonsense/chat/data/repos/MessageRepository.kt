...
class MessageRepository @Inject constructor(
    ...
    /** Live, time-ordered message list for a chat. */
    fun observeMessages(chatId: String): Flow<List<Message>> = channelFlow {
        val byId = HashMap<String, Message>()
        docs.whereEq(Tables.MESSAGES, "chat_id", chatId, orderBy = "at", ascending = true)
        ...
        realtime.changes(Tables.MESSAGES).collect { change ->
            when (change) {
                is RowChange.Upsert -> {
                    ...
                    if (msg.chatId == chatId) byId[msg.id] = msg
                }
