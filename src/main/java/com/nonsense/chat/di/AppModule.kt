...
object AppModule {
    ...
    /** Resolves IPv4 before IPv6 (broken-IPv6 path stalls), and caches the first good result for the
     *  process so a later system-DNS hang (common on this flaky device) can't re-stall every request. */
    private val Ipv4FirstDns = object : Dns {
        ...
        override fun lookup(hostname: String): List<InetAddress> {
            ...
            if (resolved.isNotEmpty()) cache[hostname] = resolved
            return resolved
        }
    }
}
